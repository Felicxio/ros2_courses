...
class TurtleControllerNode(Node): 
    def __init__(self):
        ...
        self.catch_closest_turtle = self.get_parameter(
            "catch_closest_turtle").value
    ...
    #this allows us to chose between catching the closest turtle or the first turtle in the list
    ...
    def control_loop(self):
        ...
        if distance > 0.5:
            #position
            cmd.linear.x = 2*distance #the 2 multiplied here makes the turtle move in a better way (tested)

            #orientation
            #we will need to calculate the arctg, to be able to know which angle will be set towards the goal
            goal_theta = math.atan2(dist_y, dist_x)
            diff = goal_theta - self.pose_.theta
            #to make sure that the angle is between pi and -pi
            if diff > math.pi:
                diff -= 2*math.pi
            elif diff < -math.pi:
                diff += 2*math.pi
            cmd.angular.z = 6*diff #same principle here on why the 6 is multiplied, it makes fhe movement smoother
        else:
            #target reached
            cmd.linear.x = 0.0
            cmd.angular.z = 0.0
    ...
    def callback_catch_turtle(self, future, turtle_name):
        ...
        if not response.sucess:
            self.get_logger().error(f"Turtle {turtle_name} couldn't be removed...")
